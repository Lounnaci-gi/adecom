...
// API endpoint pour récupérer les créances Eau
app.get('/api/abonnes/creances-eau', async (req, res) => {
  try {
    ...
    // Exécuter la requête SQL pour obtenir la somme des créances Eau
    const sqlQuery = `SELECT SUM(MONTTC) AS Sum_MONTTC FROM FACTURES WHERE TYPE = 'E' GROUP BY TYPE`;
    const result = await dbfSqlService.executeSelectQuery(sqlQuery);
    ...
    console.error('Erreur lors du calcul des créances Eau:', error);
    res.status(500).json({ 
      error: 'Erreur serveur lors du calcul des créances Eau',
      message: (error as Error).message
    });
  }
});
